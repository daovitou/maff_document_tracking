 <flux:sidebar sticky collapsible="mobile"
     ...
     <flux:sidebar.nav>
         ...
         @if (Gate::forUser(auth('admin')->user())->allows('view-document'))
             <flux:sidebar.group expandable :expanded="request()->routeIs('admin.doc.*')" icon="document-text"
                 heading="{{ __('Documentation') }}" class="grid">
                 @if (Gate::forUser(auth('admin')->user())->allows('view-document'))
                     <flux:sidebar.item href="{{ route('admin.doc.index') }}"
                         :current="request()->routeIs('admin.doc.index')" wire:navigate>{{ __('Document List') }}
                     </flux:sidebar.item>
                 @endif
                 @if (Gate::forUser(auth('admin')->user())->allows('create-document'))
                     <flux:sidebar.item href="{{ route('admin.doc.create') }}"
                         :current="request()->routeIs('admin.doc.create')" wire:navigate>{{ __('New Document') }}
                     </flux:sidebar.item>
                 @endif
             </flux:sidebar.group>
         @endif
         ...
         @if (Auth::guard('admin')->user()->is_system)
             <flux:sidebar.group expandable :expanded="request()->routeIs('admin.setting.*')" icon="cog-8-tooth"
                 ...
                 <flux:sidebar.item href="{{ route('admin.setting.role.index') }}">{{ __('Role') }}
                 </flux:sidebar.item>
             </flux:sidebar.group>
         @endif
