...
new #[Layout('layouts::admin.app'), Title('Create Document')] class extends Component {
    ...
    {
        return BeDocument::mainGroup($this->search)->orderBy($this->sortField, $this->sortDirection)->paginate($this->perPage);
    }
...
<div>
    <flux:heading size="xl" level="1">{{ __('BE Document List') }}</flux:heading>
                                        ...
                                        <tbody>
                                            @foreach ($items as $item)
                                                <tr class="border-t border-zinc-200 dark:border-white/10">
                                                    ...
                                                    <td>{{ Carbon::parse($item->respect_at)->format('Y-m-d') }}</td>
