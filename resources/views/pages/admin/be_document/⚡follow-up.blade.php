...
new #[Layout('layouts::admin.app'), Title('Create Document')] class extends Component {
    ...
    {
        return BeDocument::search($this->search)->where('respect_at', '<=', Carbon::now())->orderBy($this->sortField, $this->sortDirection)->paginate($this->perPage);
    }
    ...
    <table class="min-w-full table mt-6">
        <thead class="">
            <tr>
                ...
                <th class="text-left" wire:click="doSort('respect_at')">
                    <span class="flex items-center justify-between">
                        <x-datatable-header displayName="{{ __('Respect At') }}" field="send_at" :sortField="$sortField"
                            :sortDirection="$sortDirection" />
                    </span>
                </th>
                <th>{{ __('Actions') }}</th>
        ...
        <tbody class="">
            @if (count($this->docs) < 1)
                <tr>
                    <td colspan="9" class="text-center font-semibold bg-zinc-100">
                    ...
                    <tr wire:key="{{ $doc->send_to_id }}">
                        <th class="text-left">
                            <flux:tooltip content="{{ __('Edit') }}">
                                <a href="{{ route('admin.be-document.edit', $doc->id) }}"
                                    class="cursor-pointer hover:text-primary-content/20 underline flex items-center gap-0.5"
                                    wire:navigate>
                                    {{ $doc->code }}
                        ...
                        <td>{{ Carbon::parse($doc->respect_at)->format('Y-m-d') }}</td>
                        <td class="flex items-center gap-3">

                            @if (Gate::forUser(auth('admin')->user())->allows('view-be-document'))
                                <flux:tooltip content="{{ __('View') }}">
                                    <a href="{{ route('admin.be-document.send-to.view', ['id' => $doc->id, 'send_to_id' => $doc->send_to_id]) }}"
                                        class="cursor-default" wire:navigate>
