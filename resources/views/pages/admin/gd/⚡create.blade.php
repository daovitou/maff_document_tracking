...
use App\Models\Gd;
new #[Layout('layouts::admin.app'), Title('Departments | New Department')] class extends Component {
    //
    public $gd;
        ...
        return [
            'gd.name' => [
                ...
                function ($attribute, $value, $fail) {
                    $exists = Gd::whereRaw('LOWER(name) = ? AND is_active = ?', [strtolower($value), true])->exists();
                    if ($exists) {
    ...
    {
        $this->validate();
        $this->gd->created_by = Auth::guard('admin')->user()->id;
        $this->gd->save();
...
<div>
    <flux:heading size="xl" level="1">{{ __('New General Department') }}</flux:heading>
    {{-- <flux:text class="mb-6 mt-2 text-xl">{{ __('New General Department') }}</flux:text> --}}
